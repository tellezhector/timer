...
def main():
    ...
    match button:
        ...
        case Button.MIDDLE:
            ...
            if read_input_command:
              input = subprocess.check_output(read_input_command, shell=True, encoding='utf-8')
    ...
    if start_time > 0 and remaining == 0 and alarm_command:
        subprocess.call(alarm_command.format(start_time=time_format.seconds_to_text(start_time)), shell=True)
...
if __name__ == '__main__':
    try:
      ...
      logging.debug(f"{os.getenv('PATH')=}")
      logging.debug(f"{os.getenv('PWD')=}")
      main()
